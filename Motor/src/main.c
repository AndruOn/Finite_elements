 ...
 * ==========================================================================
 * 
 *  Simulation par �l�ments finis d'un motor � reluctance variable
...
//    - "theTime" est le temps courant de l'application (sauf si il a �t� remis a zero :-)
//    - Le facteur entre le temps r�el et le temps de l'application permet de controler la vitesse d'execution
...
//
// Calcul d'une it�ration temporelle 
// C'est ici qu'on ex�cute le projet :-)
//
//   Calcul du potentiel magn�tique A
//   Calcul du couple
//   Calcul de omega par l'�quation de Newton
...
// Attention : la structure de donn�es du probl�me est fig�e et vous ne pouvez pas
 ...
 * - initialise les incnnues theMotor->a à X pour voir l'animation
 * - Fixe les paramètres matériels
 * - Fixe les paramètres non-linéaires.
 */
...
//
    ...
    for (int i=0; i < theMotor->size; i++)                      // mets tout à 0
        theMotor->movingNodes[i] = 0;
    for (int i=0; i < theMesh->nElem; i++) {
        int domain = theMesh->domain[i];                       // Récupère le domaine de l'element
...
/**
 * Parse le fichier texte et genère le mesh. Sèpare les diffèrent domaine pour remplire la structure motorMesh. 
 */
...
/**
 * UTILSE à la visualisation.
 * A partir d'un motorMesh retourne un femMesh du ième domaine.
 */
