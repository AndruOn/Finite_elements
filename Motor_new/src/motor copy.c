...
char* stats_filename = "../data/EF.csv";
FILE* file;

//rapport 38:40 jerome
...
int fill_stats_file(char* stats_filename,struct stats* stats){
    FILE* file = fopen(stats_filename,"w+");
    if(file == NULL){
        printf("ERROR: coulnd fopen(stats_filename%s","");
        return -1;
    }
    fprintf(file,"couple,theta,omega\n");
    fprintf(file,"%f,%f,%f\n",stats->couple,stats->theta,stats->omega);
    
    
    if(fclose(file) !=0){
        printf("ERROR: close(stats_file did an error%s","");
        return -1;
    }
    return 0;
}
    
int write_stats(double couple){
    if(file == NULL){
        ...
        fprintf(file,"time,couple,theta,omega,coil,offset\n");
        theMotor_->omega = 0.0;
        printf("FILE STATS CREATED + ");
    }

    
    fprintf(file,"%f,%f,%f,%f,",theMotor_->time,couple,theMotor_->theta,theMotor_->omega);
    printf("PRINTED\n");
    return 0;
}
...
void motorAdaptMesh_simple(motor *theMotor, double delta)
{
    ...
    printf("rotornodes: ");
    for(int i = 0;i < len_rotorNodes;i++){
        printf("%d, ",rotorNodes[i]);
    }printf("\nstatornodes: ");
    for(int i = 0;i < len_statorNodes;i++){
...
double motorComputeCouple(motor *theMotor){
    ...
    for(iElem = first_rotorgap; iElem < last_rotorgap; iElem++) {
        ...
        for (size_t i_node = 0; i_node < 3; i_node++){
            xLoc = x[0] * (1- xsi[i] - eta[i]) + x[1] * xsi[i] + x[2] * eta[i];
            yLoc= y[0] * (1- xsi[i] - eta[i]) + y[1] * xsi[i] + y[2] * eta[i];
            double r = sqrt(pow(xLoc,2) + pow(yLoc,2));;
            double dadtheta_dadr = (pow(10,7)/r) * (dadx * xLoc + dady * yLoc) * (dady * xLoc - dadx * yLoc);;
...
//tous le PI/6 change de bobine avec un offset sur l'angle
...
void motorComputeCurrent(motor *theMotor)
{
    ...
    theta += prem_quadr;
    printf("4: offset:%f theta mod pi/2:%f ", offset, theta);
   
    if(0.0 <= theta && theta < M_PI/6){ //coils B 
        char coil = 'B';
        printf(" -> %f<=theta<%f Coil %c\n",0.0,M_PI/6,coil);
        fprintf(file,"%c,%f\n",coil,offset);
        ...
        printf("ERROR: pas dans le premier quadrant---------------------\n");
    }
